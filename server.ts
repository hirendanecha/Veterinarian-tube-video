...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/veterinarian-tube-video/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/veterinarian-tube-video/browser'), 'index.html')
    )
    .toString();
